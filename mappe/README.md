...
**Template base**, da riusare cambiando `[N]`, `[TITOLO]` e `[SOGGETTO E DETTAGLI SPECIFICI QUI]`:
...
```
The subject is a detailed straight-on architectural front elevation (not a 3D perspective, not a floor plan, a flat facade drawing like an architect's elevation) of a Romanesque abbey church facade: a stone bell tower to one side, a large rose window, a carved wooden double portal with stone archivolts, small saint statue niches, modest buttresses. A small caption beneath reads 'Facciata della Chiesa Abbaziale'.
```

**Tavola IV — Il Chiostro** (vista prospettica artistica, non piatta):

```
The subject is an atmospheric angled perspective view (an artistic illustration, not a flat plan) looking into a covered stone cloister walkway with round arches and slender columns, surrounding a small garden courtyard with a central stone fountain and a worn bench, soft morning light filtering through the arcade, a couple of small hooded monk figures in the middle distance for scale. A small caption beneath reads 'Il Chiostro'.
```

**Tavola V — Veduta Generale dell'Abbatīa** (vista d'insieme dall'alto, pittorica, coerente con mappa e planimetria):

```
The subject is a grand painterly bird's-eye view of the entire walled abbey complex from a slightly elevated angle: the church, the cloister, the dormitory, the abbot's separate large house, the pond and vegetable garden, nestled among dark fir forest on a mountain pass, gatehouse facing south-southwest with a path descending toward a distant village, consistent with the existing regional map and floor plan. A small caption beneath reads 'Veduta Generale dell'Abbatīa'.
```
